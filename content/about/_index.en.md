...
- **Key Contributions**
  ...
  - Designed Dual Write + validation batch for data consistency during transition
  - Applied Hexagonal Architecture + CQRS to remove technical dependencies from domain logic
  - Built FeignClient sync communication, AWS SQS event processing
- **Results**
...
- **Key Contributions**
  ...
  - Removed Spring Cloud Eureka/Gateway dependencies → Kubernetes native
  - Built GitHub Actions + ArgoCD CI/CD pipeline
  - Designed and built Grafana LGTM Stack monitoring (sidecar pattern)
- **Results**
...
- **Key Contributions**
  ...
  - Codified AWS infrastructure (EC2, RDS, VPC) with Terraform
- **Results**
...
- **Key Contributions**
  ...
  - Dynamic DataSource routing with AbstractRoutingDataSource + AOP
  - [Redis caching]({{< relref "/blog/backend/performance/look-aside-cache-api-perf" >}}), [covering index]({{< relref "/blog/architecture/jpa-sql-ideology-and-gap" >}}), event-based async processing
- **Results**
...
- **Key Contributions**
  ...
  - Resolved Race Condition with Redisson distributed lock
