...
진짜 자동화라고 하기엔 뭔가 부족했습니다. 이건 "반"자동화였습니다. 자동화를 위해서는 에러가 발생했을 때 AI가 스스로 인지하고 시작할 수 있어야 했습니다.
...
여기서 눈치채신 분들도 있겠지만, 이렇게 되면 **에러 알림 → AI 에이전트 자동 분석 → PR 생성**까지 자동화가 가능해질 거라 판단했습니다.

```mermaid
flowchart TB
    subgraph After["OpenClaw (자동화)"]
...
처음에는 Cron Job이 안정적이라 매력적이었습니다. OpenClaw 프로세스와 독립적으로 실행되니까요.

하지만 Heartbeat가 설정이 훨씬 간단하다는 점이 컸습니다. 그리고 "OpenClaw가 죽으면 Heartbeat도 멈춘다"는 단점은 사실 문제가 아니었습니다. OpenClaw는 설치 시 **OS 레벨의 서비스**(macOS LaunchAgent, Linux systemd 등)로 등록되기 때문에 프로세스가 종료되거나 PC를 재부팅해도 자동으로 다시 실행됩니다.

그래서 **Heartbeat를 선택했습니다.** 설정이 간단하고, OpenClaw 자체가 프로세스 복구를 해주니 Cron Job 못지않게 안정적이었습니다. Heartbeat 주기는 30분으로 설정해 30분마다 에러를 체크하도록 했습니다.

너무 자주 Heartbeat를 돌리게 되면 토큰 비용이 많이 들 수 있기 때문에 30분 주기가 적절하다고 판단했습니다.
...
flowchart TB
    subgraph "일반 상황 (자동)"
        Cron1["⏰ 30분마다<br/>Heartbeat"] --> Auto["자동 처리"]
    end
...
**일반 상황**: Heartbeat가 30분마다 자동으로 폴링해서 처리
...
- **3:30** - Heartbeat가 Loki에서 새 에러 감지
...
| 과제 | MCP (반자동화) | OpenClaw (자동화) |
|------|---------------|---------------------|
| 1. 에러 인지 | ❌ 수동 트리거 | ✅ Heartbeat 자동 폴링 |
| 2. 로그/트레이스 조회 | ✅ | ✅ |
| 3. 원인 분석 | ✅ | ✅ |
| 4. 이슈 생성 | ✅ | ✅ |
| 5. 코드 수정 | ❌ | ✅ |
...
에러 자동화가 안정적으로 동작하자, 같은 패턴을 **성능 병목 추적**에도 적용했습니다. 모니터링 메트릭에서 응답 속도가 느린 API를 자동으로 탐색하고, 트레이스 분석을 통해 병목 구간을 찾아 개선 방향을 제안하도록 자동화했습니다. 이 부분에 대해서는 기회가 있으면 별도의 글에서 자세히 다뤄보겠습니다.
...
솔직히 말하면 이런 자동화를 만들면서도 계속 드는 생각이 있습니다. **AI가 이렇게 발전하면 나같은 주니어 개발자는 어떻게 되는 걸까?**

지금은 제가 AI를 활용해서 생산성을 높이고 있지만, 회사 입장에서는 AI만으로도 충분하다고 판단할 수 있지 않을까요? 코드 짜는 일은 AI가 하고, 리뷰와 의사결정은 소수의 시니어 개발자들만 있으면 되는 거니까요.

무섭기도 하지만 어쩔 수 없이 이 흐름에 올라타야 한다고 생각합니다. AI를 거부하면 도태될 것 같거든요. 그래서 이렇게 적극적으로 도입해보려 노력하고 있습니다.

AI와 경쟁하는 게 아니라 AI를 잘 활용하는 개발자가 되는 게 살아남는 길이라고 믿고 싶습니다.
