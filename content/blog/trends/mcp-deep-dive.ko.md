...
Tools의 가장 중요한 특징은 **Model-Controlled**라는 점입니다. 사람이 "이 도구를 써"라고 명시적으로 지시하는 게 아니라, **LLM이 상황을 추론해서 알아서 도구를 선택하고 호출**합니다.

물론 이게 가능하려면 LLM이 도구를 잘 이해해야 합니다. 그래서 `description`을 명확하게 작성하는 것이 중요합니다. "날씨를 가져옵니다" 보다는 "지정된 도시의 현재 날씨 정보(온도, 상태)를 조회합니다"처럼 구체적으로 작성하는 게 추론 정확도를 높입니다.
...
3. **LLM이 스스로 판단합니다**: LLM은 **Tool Use(또는 Function Calling)**라는 능력을 학습하고 있습니다. 사용자의 질문과 주어진 도구 목록을 보고, "이 상황에서 어떤 도구를 어떤 파라미터로 호출해야 하는가"를 추론합니다.
...
단순 HTTP 요청/응답으로도 통신은 가능합니다. 하지만 MCP에서는 **서버가 클라이언트에게 먼저 메시지를 보내야 하는 경우**가 있습니다. 다음과 같은 상황들입니다.
- 도구 목록이 변경되었을 때 알림
- 긴 작업의 진행 상황 업데이트
...
특히 MCP 서버가 민감한 데이터(데이터베이스, 파일 시스템 등)에 접근할 수 있다면 인증은 필수죠.
...
개인적으로 사용하는 MCP 서버라면 **stdio**로 충분하다고 생각합니다. 팀에서 공유하거나 여러 환경에서 접근해야 한다면 **Streamable HTTP**를 고려해볼 수 있겠죠.
...
이 기능 덕분에 **동적인 MCP 서버**를 만들 수 있습니다. 새로운 플러그인이 설치되면 도구 목록을 업데이트하거나, 설정 파일이 변경되면 리소스를 업데이트하거나, 데이터베이스 스키마가 변경되면 알림을 보내는 식이죠.
...
MCP 없이 직접 구현하는 방식입니다. 많은 분들이 AI 챗봇 백엔드를 구축할 때 이런 식으로 하셨을 것 같습니다.
...
이 방식은 백엔드가 사용자 질문을 분석해서 어떤 서비스를 호출할지 결정하고 각 서비스마다 다른 REST API 인터페이스를 호출하며 새로운 서비스를 추가할 때마다 백엔드 라우팅 로직 수정이 필요해 집니다.

이처럼 작동은 합니다. 하지만 서비스가 늘어날수록 몇 가지 문제가 생길 수 있죠.
...
flowchart TB
    ...
    subgraph Backend["AI 챗봇 백엔드 서버 (MCP Host)"]
        API["API 서버"]
...
한번 MCP를 이용하는 MCP 서버 구성도를 그려봤습니다. 아키텍처로 보면 아무래도 이해가 더 쉬울 것 같아서요.
...
물론 모든 서비스에 MCP가 필요한 건 아니라고 생각합니다.
...
2025년에 여러 보안 연구자들이 MCP 관련 보안 이슈를 발표했습니다.
...
[Trend Micro 연구](https://www.trendmicro.com/vinfo/us/security/news/cybercrime-and-digital-threats/mcp-security-network-exposed-servers-are-backdoors-to-your-private-data)에서는 인터넷에 노출된 MCP 서버 492개를 발견했는데, **인증이나 암호화 없이** 운영되고 있었습니다. 이 서버들은 총 1,402개의 도구에 접근할 수 있었고, 90% 이상이 데이터 소스에 직접 읽기 권한을 제공하고 있었습니다. [Bitsight 보고서](https://www.bitsight.com/blog/exposed-mcp-servers-reveal-new-ai-vulnerabilities)에서도 약 1,000개의 MCP 서버가 인증 없이 공개 인터넷에 노출되어 있다고 경고했습니다.

또한 [Astrix의 MCP 보안 현황 보고서](https://astrix.security/learn/blog/state-of-mcp-server-security-2025/)에 따르면, MCP 서버의 88%가 자격 증명을 요구하지만, 그중 53%는 장기간 유효한 정적 API 키나 PAT(Personal Access Token)에 의존하고 있었습니다.

이런 연구 결과들을 보면, MCP를 사용할 때는 보안에 특히 신경 써야 한다는 생각이 들었습니다.
...
이 글을 정리하면서 느낀 점이 있습니다. MCP가 해결하려는 문제는 사실 새로운 게 아니었던 것 같습니다. **추상화, 관심사의 분리, 인터페이스 기반 설계**. 저희가 좋은 소프트웨어를 만들기 위해 오랫동안 고민해온 원칙들이 AI 도구 통합이라는 새로운 영역에 적용된 것뿐이라는 생각이 듭니다.
