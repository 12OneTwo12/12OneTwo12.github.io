...
물론 기존 Sub-Agent도 독립된 컨텍스트로 병렬 실행은 가능했습니다. 하지만 Sub-Agent는 결과를 메인 에이전트에게만 보고하는 구조라, 분석 도중에 팀원끼리 "이 서비스가 네 쪽 DB 스키마랑 연관 있는데 확인해봐"같은 소통이 안 됐죠. Agent Teams는 팀원끼리 직접 메시지를 교환하고 공유 Task 리스트로 자율 조율하니까, **분석 결과의 정합성이 눈에 띄게 올라갔습니다.** Team Lead가 각 분석 결과를 취합해서 통합 문서를 만들어주는 과정도 깔끔했고요.

별도 도구 설치 없이 설정 하나로 네이티브 멀티 에이전트를 바로 쓸 수 있다는 점도 좋았습니다.

다만 아쉬운 점도 있었습니다. oh-my-claudecode는 32개의 전문화된 에이전트(planner, architect, critic 등)와 7가지 실행 모드가 **검증된 패턴으로 미리 정의**돼 있어서 바로 쓸 수 있는데, Agent Teams는 팀 구성을 LLM이 매번 즉석에서 판단합니다. 물론 "이거 해줘"라고 하면 알아서 팀을 짜주긴 하지만, oh-my-claudecode의 Ultrapilot 모드처럼 **이미 최적화된 패턴을 바로 적용하는 경험**과는 차이가 있었습니다.
