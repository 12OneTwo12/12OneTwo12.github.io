...
이때 회사 서비스는 단일 인스턴스로 구성돼 있었습니다. 그러나 서비스가 늘면서 메모리 부족으로 죽거나, 한 서비스가 CPU를 독점해 옆 서비스까지 느려지는 일이 잦았습니다. 기존에 회사는 개발 서버조차 없어 테스트를 운영 환경에서 해오고 있었습니다. (이때 꽤나 충격을 받았습니다. 개발 환경은 당연하다고 생각했어서) 6월부터 [쿠버네티스(GKE)로 옮기면서]({{< relref "blog/infrastructure/docker-compose-to-k8s" >}}) Eureka·게이트웨이를 걷어내고, ArgoCD로 GitOps를 구성했습니다.
...
덕분에 장애 감지가 수 시간에서 수 분으로 줄었습니다. 실제로 [네이밍이 컨벤션이 지켜지지 않아 발생하고 있던 예외]({{< relref "blog/culture/naming-convention" >}}) 같은 문제를, 찾게 되기도 했습니다.
