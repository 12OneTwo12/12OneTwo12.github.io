...
RPC의 핵심은 **네트워크 통신의 복잡함을 숨기는 것**이라고 생각합니다.
...
JSON-RPC는 RPC 패러다임을 JSON 메시지 포맷으로 구현한 프로토콜입니다. 스펙 문서가 몇 페이지 안 될 정도로 단순하더라구요.
...
gRPC가 JSON 기반 API보다 빠른 건 맞지만, **모든 상황에서 의미 있는 차이는 아닌 것 같습니다.**

- 네트워크 지연이 큰 환경: 직렬화 속도보다 네트워크가 병목
- 단순한 CRUD: 오버엔지니어링일 수 있음
- 브라우저 클라이언트: gRPC-Web 추가 복잡성

gRPC의 진가는 **대량의 마이크로서비스 간 통신**, **스트리밍이 필요한 경우**, **스키마 관리가 중요한 경우**에 발휘되는 것 같습니다.
...
**진짜 차이는 "누가 주도권을 갖고 무엇을 표준화하느냐"라고 생각합니다.**
...
여기까지 오면 솔직히 말해야 할 것 같은데요. RPC의 가장 큰 부작용이 있습니다.
...
RPC가 네트워크를 너무 잘 숨기면, 분산 시스템의 실패 모델을 무시한 코드가 양산될 수 있습니다. Google 내부에서 RPC 문화가 잘 작동하는 건 엄격한 SRE 규율이 함께 있기 때문이지, RPC 자체가 마법은 아니라고 생각합니다.
