...
얼마 전 [MCP(Model Context Protocol) 글]({{< relref "/blog/trends/mcp-deep-dive" >}})을 쓰면서 JSON-RPC를 다루게 되었습니다. 그런데 글을 쓰다 보니 스스로에게 질문이 생기더군요.

**"JSON-RPC가 경량화됐다고 하는데, HTTP 위에서 REST랑 뭐가 다르지?"**
...
이 아이디어는 1980년대부터 있었습니다. Sun Microsystems의 ONC RPC, 이후 CORBA, Java RMI 등 다양한 구현이 있었죠. 지금의 gRPC나 JSON-RPC도 이 오래된 아이디어의 연장선에 있습니다.
...
이 부분이 헷갈리기 쉬운데, 각각의 개념을 명확히 분리해보겠습니다.
...
### JSON-RPC: 전송 방법이 자유롭다

JSON-RPC 공식 스펙에는 이렇게 명시되어 있습니다:
...
비유하자면, RPC는 "객체지향 프로그래밍"같은 개념이고, JSON-RPC나 gRPC는 "Java", "Python"같은 구체적인 구현인 셈입니다.
...
REST가 더 직관적이고 널리 쓰이지만, 복잡한 작업을 표현할 때는 어색해질 수 있습니다.
...
예를 들어, 코틀린에서 이런 식으로 외부 API를 감싸는 모듈을 만들었다고 해봅시다.
...
SDK는 기본적으로 **REST API를 쓰기 쉽게 감싼 라이브러리**입니다.
...
### SDK로 RPC 흉내는 낼 수 있지만

SDK로 RPC처럼 쓸 수는 있습니다. 하지만:
...
> **SDK는 "편의성 계층"이고, RPC는 "시스템 간 계약과 사고방식"입니다.**

SDK로 RPC처럼 쓸 수는 있지만, 계약의 강도, 진화 방식, 실패 감지 시점이 전부 다릅니다.

그래서 **"직접 만든 SDK는 RPC 패러다임을 수동으로 구현한 것"**이라고 볼 수도 있지만, 엄밀히 말하면 **RPC의 핵심인 "강한 계약"이 빠진 흉내**에 가깝습니다. 물론 그게 나쁘다는 건 아닙니다. 상황에 따라 SDK + REST가 더 적합할 수 있고, 오히려 네트워크를 의식하게 해주는 게 장점일 수도 있습니다
