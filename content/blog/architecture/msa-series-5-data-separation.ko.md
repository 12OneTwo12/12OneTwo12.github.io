...
하지만 아직 해야 할 고민이 훨씬 많이 남아있습니다. [2편]({{< relref "/blog/architecture/msa-series-2-boundary" >}})에서 서비스를 어떻게 나눌지 고민했고, [3편]({{< relref "/blog/architecture/msa-series-3-communication" >}})과 [4편]({{< relref "/blog/architecture/msa-series-4-resilience" >}})에서 나눠진 서비스들이 어떻게 통신하고 장애에 대응하는지 살펴봤습니다. 이제는 MSA 환경에서 가장 골치 아픈 문제 중 하나인 **데이터 분리 문제**에 대해 이야기해보려 합니다. 실제로 제가 MSA를 도입하면서 가장 많이 고민하게 됐던 부분입니다.
...
[1편]({{< relref "/blog/architecture/msa-series-1-introduction" >}})에서도 이야기 했듯이 모든 기술은 보통 **특정한 문제**를 해결하기 위해 도입됩니다. 그렇다면 MSA에서 DB를 분리하지 않았을 때 어떤 **특정한 문제**들이 발생할 수 있는지 살펴보면 꼭 분리해야하는 지 여부를 쉽게 판단할 수 있을 것 같습니다.
...
[3편]({{< relref "/blog/architecture/msa-series-3-communication" >}})에서 장애 전파에 대해 이야기했는데, DB를 공유하면 여기서도 문제가 생깁니다. 하나의 서비스에서 무거운 쿼리를 날리거나 Lock을 오래 잡으면, 다른 서비스들도 영향을 받습니다.
...
또 다른 장점으로는 **Polyglot Persistence**가 가능해진다는 점입니다. 서비스마다 자기만의 DB를 가지니까, 각 서비스의 특성에 맞는 DB 기술을 선택할 수 있습니다. 주문 서비스는 트랜잭션이 중요하니 RDB를, 검색 서비스는 전문 검색이 필요하니 Elasticsearch를, 세션 저장소는 빠른 읽기/쓰기가 필요하니 Redis를 사용하는 식이죠. 공유 DB에서는 모든 서비스가 하나의 DB 기술에 종속되지만, DB를 분리하면 이런 유연성을 얻을 수 있게 됩니다.
...
Schema per Service는 물리적으로는 하나의 DB 인스턴스를 사용하지만, 서비스마다 별도의 스키마로 논리적 분리를 하는 방식입니다. 운영 비용을 줄일 수 있고 초기 단계에서 부담이 적습니다. 같은 DB 인스턴스이기 때문에 Cross-schema JOIN이 가능하고 트랜잭션도 원하면 묶을 수 있습니다. MySQL이든 PostgreSQL이든 동일한 인스턴스 내 스키마 간에는 별도 설정 없이 Cross-schema JOIN과 트랜잭션이 가능합니다. 

참고로 PostgreSQL에서 `dblink`나 `postgres_fdw`가 필요한 건 스키마가 아닌 **다른 데이터베이스**에 접근할 때입니다. PostgreSQL에서 "스키마"와 "데이터베이스"는 다른 개념이거든요.
...
- **장애 전파**: [3편]({{< relref "/blog/architecture/msa-series-3-communication" >}})에서 다뤘던 문제죠. 회원 서비스가 죽으면 주문 조회도 안 됩니다. 회원 이름 하나 때문에 전체가 실패하는 거죠. 물론 장애 격리 패턴을 적용할 수는 있지만, 의존성이 있다는 사실 자체는 변하지 않습니다.
- **N+1 문제**: 주문 목록 조회를 생각해보세요. 주문 10개를 조회하면 회원 API 10번, 상품 API 10번... 성능에 영향을 미치게 될 가능성이 높습니다. 물론 Batch API(예: `GET /users?ids=1,2,3,4,5`)를 만들어서 한 번에 여러 건을 조회하는 방식으로 완화할 수 있지만 이렇게 되면 API 설계가 복잡해지고, 모든 상황에 적용 가능한 것도 아니라서 근본적인 해결책이라고 보기는 어려운 것 같습니다.
...
- **데이터 불일치 가능성**: 이벤트가 지연되면 잠시 동안 오래된 데이터가 보일 수 있습니다. 이벤트 유실로 인해 데이터 정합성 문제가 발생할 수도 있고요. (이 문제는 Transactional Outbox 패턴이나 Kafka 같은 신뢰성 있는 메시지 브로커를 사용해서 해결할 수 있는데, 자세한 내용은 [6편]({{< relref "/blog/architecture/msa-series-6-event-consistency" >}})에서 다루겠습니다.)
...
그리고 쓰기 후 읽기 모델에 반영되기까지 시간차가 있습니다. "방금 수정했는데 왜 안 바뀌어 있지?"라는 상황이 발생할 수 있죠. 이벤트 처리, 실패 시 재처리 등도 고려해야 합니다. 이 부분은 [6편]({{< relref "/blog/architecture/msa-series-6-event-consistency" >}})에서 좀더 자세히 다루도록 하겠습니다.
...
그래서 저는 데이터 분리를 고민할 때마다 이 질문을 먼저 던져야 한다고 생각합니다. **"정말 이 데이터가 분리되어야 하는가?"**

기술적으로 분리할 수 있다고 해서 분리해야 하는 건 아니니까요. 함께 변경되고, 함께 조회되고, 함께 트랜잭션이 필요한 데이터라면 애초에 같은 곳에 있어야 하는 게 아닌지 고민해볼 필요가 있다고 생각합니다.
