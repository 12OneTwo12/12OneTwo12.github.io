...
저는 2025년 3월에 팀에 합류했는데, 합류 시점에 팀은 레거시 모놀리식 서비스를 MSA로 전환하는 과정 중에 있었습니다. 당시 개발자는 총 5명, 그 중 백엔드는 저 포함 2명이었습니다. 16개 서비스가 각각 개별 Git 레포지토리로 관리되고 있었는데, 이 인원으로 16개 레포를 관리한다는 건 맞지 않다고 생각했습니다.
...
혼자서 13개가 넘는 서비스를 운영하면서 기능 개발까지 한다는 건 생각보다 쉽지 않았습니다. 서비스 간 호출 관계를 파악하는 것도, 장애가 발생했을 때 어떤 서비스에서 시작된 문제인지 추적하는 것도, 배포 파이프라인을 13개나 관리해야 하는 것도 부담으로 다가왔습니다. 또 Config Server, Elastic Search, Redis와 같은 공통 인프라 서비스도 따로 운영해야 했죠.
...
보통 K8s, MSA 구조에서는 고가용성을 위해 서비스별로 최소 2개씩 파드를 운영하곤 합니다. 13개 서비스면 최소 26개 파드가 필요한 거죠. 여기에 위에서 언급한 공통 인프라 서비스까지 더하면 리소스는 더 늘어납니다.
...
만약 `@Lazy`가 없었다면 앱 기동 시점에 `NoSuchBeanDefinitionException`이 터지면서 배포 자체가 실패했을 겁니다. 즉, 배포 단계에서 바로 문제를 발견하고 고칠 수 있었을 텐데 말이죠. 그런데 `@Lazy`가 빈 해석을 런타임으로 미뤄버렸기 때문에 앱은 정상 기동되고, 실제로 해당 API가 호출되는 시점에서야 에러가 발생한 겁니다. 개발 서버에서 발견해서 다행이지, 운영에서 터졌으면 장애로 이어졌을 겁니다.
...
실제 GCP 비용을 비교해보니 효과가 확실했습니다. MSA로 운영하던 1월 기준 일일 비용은 Compute Engine ₩96,505 + Kubernetes Engine ₩6,885로 **약 ₩103,000 수준**이었는데, 모듈러 모놀리식 전환 후인 2월 말 기준으로는 Compute Engine ₩58,768 + Kubernetes Engine ₩6,864로 **약 ₩65,000 수준**으로 떨어졌습니다.

Compute Engine 비용이 약 39% 절감된 거죠. Kubernetes Engine 비용은 GKE 클러스터 자체 운영 비용이라 거의 변하지 않았지만, 실제 워크로드를 처리하는 Compute Engine 비용이 크게 줄었습니다. 월 단위로 환산하면 **약 ₩310만에서 ₩197만으로, 매달 약 ₩113만 정도의 비용 절감** 효과가 있었습니다.
...
별개로 전환 후에 CI 테스트 빌드 시간이 45분으로 급증하는 문제를 겪기도 했는데, 이 부분은 [후속 글]({{< relref "/blog/backend/performance/ci-test-build-optimization" >}})에서 다루고 있습니다.
...
실제로 위에서 언급한 것처럼 Compute Engine 비용이 일일 약 ₩96,000에서 ₩58,000으로 약 39% 줄었고, 월 기준으로는 약 ₩113만의 비용 절감 효과가 있었습니다. 물론 서버 비용을 더 줄일 수 있는 방안들을 더 고민해 개선할 예정입니다. 가령 K8s가 꼭 필요한가, 정말 항상 켜져 있어야 하는 서비스인가, 등등을 말이죠.
