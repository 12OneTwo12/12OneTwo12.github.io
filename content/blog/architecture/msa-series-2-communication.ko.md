...
하나의 비즈니스 목적을 이루기 위해 시스템을 설계하는 경우가 대부분일 텐데, 이런 경우 각 도메인 혹은 기능별로 나누어진 서비스들은 보통 서로 협력하거나 서로의 데이터를 필요로 하기 마련이기 때문입니다.
...
이로 인해 모놀리식 환경에서는 네트워크 통신의 오버헤드가 없고, 데이터 일관성 유지가 상대적으로 용이하다는 장점이 있습니다.
...
겉은 비동기인데 속은 동기인 셈이죠. 결국 동기냐 비동기냐는 "얼마나 느슨하게 결합할 수 있는가"의 문제라고 생각합니다.
...
`.proto` 파일에서 `stream` 키워드로 스트리밍 여부를 정의합니다. (`returns (stream Order)` 형태)
...
**주의점**: Gateway가 Single Point of Failure가 될 수 있습니다. Gateway 장애 = 전체 시스템 마비이므로, 다중화와 헬스체크는 필수입니다.

---

## 비동기 통신: Message Queue vs Event Broker

이번에는 비동기 통신에 대해 살펴보겠습니다. 비동기 통신에서 가장 많이 언급되는 것이 **Message Queue**와 **Event Broker**입니다.

두 개념이 혼용되어 사용되는 경우도 있는데, 엄밀히 말하면 서로 다른 패턴과 사용 목적을 가지고 있습니다.

### Message Queue (SQS, RabbitMQ)

Message Queue는 **점대점(Point-to-Point)** 통신 패턴이라고 할 수 있습니다. 하나의 메시지는 하나의 소비자(Consumer)만 처리합니다.
...
Message Queue의 대표적인 구현체로는 AWS SQS, RabbitMQ 등이 있습니다.
...
이때 주문 서비스가 "주문 완료" 이벤트를 Event Broker에 발행하면, 알림 서비스, 정산 서비스, 재고 서비스가 각각 해당 이벤트를 구독하여 자신의 작업을 수행합니다. **하나의 이벤트가 여러 서비스에 동시에 전달**되는 것입니다.
...
이 특성 덕분에 Kafka는 다음과 같은 상황에서 강력한 힘을 발휘합니다.
...
물론 Kafka도 Pub/Sub 패턴을 지원하기 때문에 Event Broker처럼 사용할 수 있습니다. 하지만 Kafka를 단순히 "여러 구독자에게 이벤트를 전달하는 도구"로만 보기보다는, **이벤트의 영속성과 재처리 가능성**이 필요한 경우에 선택하는 것이 바람직합니다.

다만 한 가지 주의할 점이 있습니다. Kafka가 강력한 도구인 것은 맞지만, 그만큼 **트레이드오프**도 존재합니다. 파티션, 오프셋, 컨슈머 그룹 등 학습해야 할 개념들이 많고, 클러스터 운영과 모니터링에 들어가는 리소스도 만만치 않습니다. 저도 처음 Kafka를 도입했을 때 "이벤트 재처리가 가능하다"는 장점에 끌렸는데, 막상 운영해보니 그 장점을 누리기 위해 감수해야 할 복잡성이 상당하더라구요. 단순한 이벤트 전파가 목적이라면 SNS+SQS 조합이 훨씬 가볍고 관리하기 편할 수 있습니다.

또 하나 반드시 알아야 할 점이 있습니다. **Kafka는 기본적으로 At-least-once 전달을 보장합니다.** 즉, 메시지가 최소 한 번은 전달되지만, 네트워크 문제나 Consumer 재시작 등으로 인해 **같은 메시지가 중복 처리될 수 있습니다.** Exactly-once 시맨틱을 지원하긴 하지만 설정이 복잡하고 성능 오버헤드가 있어서, 대부분의 경우 **Consumer 쪽에서 멱등성(Idempotency)을 보장하도록 설계**하는 것이 일반적입니다. "이벤트니까 한 번만 처리되겠지"라고 생각하면 나중에 중복 처리 버그로 고생할 수 있습니다.
...
제 개인적인 생각에 **기술은 양자택일이 아니라는 것**입니다. 실제 MSA 환경에서는 하나의 시스템 내에서 상황에 따라 동기와 비동기를 함께 사용하는 것이 일반적입니다.
...
만약 일관되지 않게 어디에선 REST API, 또 어디에선 GraphQL을 사용한다면 복잡도가 서비스 파악과 디버깅이 어려워질 수밖에 없습니다. 팀 내에서 명확한 가이드라인을 세우고, 그에 따라 일관되게 적용하는 것이 중요하다고 생각합니다.
...
이번 편에서 다룬 내용을 정리해보면 다음과 같습니다.

1. **MSA에서 서비스 간 통신은 네트워크를 통해 이루어지기 때문에, 모놀리식과는 근본적으로 다른 고려사항들이 있습니다.** 분산 컴퓨팅의 8가지 오류를 항상 염두에 두어야 한다고 생각합니다.

2. **동기 통신과 비동기 통신은 각각의 용도가 있습니다.** 즉시 응답이 필요한 경우에는 동기 통신을, 결과를 기다릴 필요가 없거나 여러 서비스에 동시에 알려야 하는 경우에는 비동기 통신을 선택하는 것이 좋다고 생각합니다.

3. **REST, gRPC, GraphQL은 각각 장단점이 있습니다.** 서비스 간 내부 통신에는 gRPC가, 클라이언트-서버 통신에는 REST API가 적합한 경우가 많다고 생각합니다.

4. **Message Queue, Event Broker, Kafka는 각각 다른 특성을 가집니다.** 점대점 통신에는 Message Queue를, 단순 이벤트 전파에는 Event Broker를, 이벤트 재처리나 영속성이 필요한 경우에는 Kafka를 선택하는 것이 바람직합니다.
...
이번 편에서는 그 선택의 갈림길들을 최대한 구조적으로 정리해보려 했습니다.

다음 편에서는 이러한 통신 방식 위에서 장애 대응 문제에 대해 이야기해보려고 합니다.
