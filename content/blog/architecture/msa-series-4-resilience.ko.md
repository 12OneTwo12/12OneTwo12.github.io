---
title: "[MSA에 관한 고찰 4/7편] 장애는 어떻게 전파되고, 우리는 어디서 끊어야 하는가"
tags:
  - msa
  - architecture
  - distributed-systems
...
전편: [[MSA에 관한 고찰 3/7편] MSA에서의 서비스간 통신]({{< relref "/blog/architecture/msa-series-3-communication" >}})
...
우선 장애는 어떻게 전파 될까요? 모놀리식 환경에서는 모든 기능이 하나의 프로세스 안에서 실행되기 때문에, 한 모듈의 장애가 전체 애플리케이션에 영향을 줄 수 있다고 [1편]({{< relref "/blog/architecture/msa-series-1-introduction" >}})에서 이야기했습니다. 그렇다면 MSA에서는 서비스가 분리되어 있으니 장애 격리가 자연스럽게 될까요?
...
[2편]({{< relref "/blog/architecture/msa-series-2-boundary" >}})에서도 "언제 쓰면 안 되는가"를 강조했듯이, 장애 대응 패턴도 마찬가지로 피해야 할 상황이 있습니다.
...
**데이터는 어떻게 분리해야 하는가?** 입니다.

다음 5편에서는 왜 DB를 분리해야 하는지, 그리고 분리했을 때 JOIN이 안 되는 문제를 어떻게 해결하는지에 대해 이야기하겠습니다.
